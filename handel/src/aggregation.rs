...
impl<P: Protocol, T: Clone + Debug + Eq + Serialize + Deserialize + Sized + Send + Sync + Unpin> NextAggregation<P, T> {
    pub fn new(
        ...
        assert!(!own_contribution.contributors().is_empty());
        debug!("BitSet.len(): {}, {:?}", own_contribution.contributors().len(), own_contribution.contributors());
        for i in own_contribution.contributors().iter() {
            debug!("own_contrinbution.contributors(): {:?}", i);
        }
