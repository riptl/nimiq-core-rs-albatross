...
impl Blockchain {
    ...
    pub fn push_block(
        ...
        if let Block::Micro(ref micro_block) = block {
            ...
            // Only check for forks in an epoch that still allows fork proofs to be submitted,
            // i.e., the block at hand must be in the current or previous epoch.
            if policy::epoch_at(micro_block.header.block_number) + 1
                >= policy::epoch_at(self.height())
            {
